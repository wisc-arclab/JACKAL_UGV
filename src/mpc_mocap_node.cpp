...
void stateCallback(const geometry_msgs::PoseStamped& msg) { current_pose = msg; }
...
bool is_target(const geometry_msgs::PoseStamped& cur, double goal_x, double goal_y)
{
	if(abs(cur.pose.position.x - goal_x) < 0.05 && abs(cur.pose.position.y - goal_y) < 0.05)
	{
		return true;
	}
	else return false;
...
int main(int argc, char **argv)
{
    ...
    ros::Subscriber state_sub = nh.subscribe("/transformed_pose", 10, stateCallback);
    ...
    while(ros::ok())
    {
		...
		// State
		double px = current_pose.pose.position.x; // state[0];
		double py = current_pose.pose.position.y; // state[1];

		double q0 = current_pose.pose.orientation.x;
		double q1 = current_pose.pose.orientation.y;
		double q2 = current_pose.pose.orientation.z;
		double q3 = current_pose.pose.orientation.w;
		...
		// Check target
		bool goal = is_target(current_pose, goal_x, goal_y) && count >= path.poses.size();
		if(goal)
		{
			vel.linear.x = 0;
			vel.angular.z = 0;
