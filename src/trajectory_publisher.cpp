...
nav_msgs::Path generate_figure_eight_trajectory(double radius, int points, std::string frame_id) {
    ...
    for (int i = 0; i < points; i++) {
        double t = (double)i / points * 4 * M_PI; // parameter t goes from 0 to 2*PI
        geometry_msgs::PoseStamped pose;
        pose.header.stamp = ros::Time::now();
        pose.header.frame_id = frame_id;
...
// Function to generate a figure-eight trajectory with rotation
nav_msgs::Path generate_figure_eight_trajectory_with_rotation(double radius, int points, double angle, std::string frame_id) {
    nav_msgs::Path path;
    path.header.frame_id = frame_id;
    path.header.stamp = ros::Time::now();
    ...
    for (int i = 0; i < points; i++) {
        ...
        double y = radius * sin(t) * cos(t);

        // Apply rotation
        double rotated_x = x * cos_angle + y * sin_angle;
        double rotated_y = -x * sin_angle + y * cos_angle;

        pose.pose.position.x = rotated_x;
        pose.pose.position.y = rotated_y;
        pose.pose.position.z = 0;  // Assuming flat ground
        ...
        // Apply rotation to the next point
        double next_rotated_x = next_x * cos_angle + next_y * sin_angle;
        double next_rotated_y = -next_x * sin_angle + next_y * cos_angle;

        double yaw = atan2(next_rotated_y - rotated_y, next_rotated_x - rotated_x);
        pose.pose.orientation = tf::createQuaternionMsgFromYaw(yaw);
...
int main(int argc, char **argv)
{
    ...
    double radius = 2.5;  // Radius of the circle
    ...
    int points = 1200;     // Number of points in the circle
    std::string frame_id = "world";  // Or the frame you want to use
    double angle = 100.0;
    ...
    nav_msgs::Path path = generate_figure_eight_trajectory_with_rotation(radius, points, angle, frame_id);
